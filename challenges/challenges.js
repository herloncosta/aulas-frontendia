...
// Criar um código para conversão de temperatura entre Celsius e Fahrenheit e o inverso.
...
function concatenarDados(nome, idade, cidade) {
...
function calcularAreaPerimetroRetangulo(ladoA, ladoB) {
...
function verificarDivisibilidade(x, y) {
  if (x % y === 0) {
    return `${x} é divisível por ${y}.`;
  }

  return `${x} não é divisível por ${y}.`;
}
...
function calcularTarifa(idade, tipoUsuario) {
  ...
  return `O valor da tarifa é R$${tarifaFinal.toFixed(2)}.`;
}
...
function somatorioAteN(n) {
  ...
  return `A soma de 1 até ${n} é ${soma}.`;
}
